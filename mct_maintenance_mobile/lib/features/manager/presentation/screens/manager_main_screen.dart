...
class _ManagerMainScreenState extends State<ManagerMainScreen> {
  ...
  void initState() {
    ...
    _notificationSubscription = FCMService().onNotificationTap.listen((data) {
      if (kDebugMode)
        debugPrint('🔔 [ManagerMainScreen] Notification tap reçue via stream');
      if (mounted) {
  ...
  Future<void> _checkPendingNotifications() async {
    ...
    if (notificationData != null) {
      if (kDebugMode)
        debugPrint(
            '📬 [Manager] Notification en attente détectée, navigation...');
      final navigationService = NotificationNavigationService();
    ...
    return GestureDetector(
      ...
      child: CircleAvatar(
        ...
        foregroundImage: _user?.profileImage != null
            ...
            : null,
        child: Text(
          (_user?.firstName?.isNotEmpty == true ? _user!.firstName![0] : 'M')
              .toUpperCase(),
          style: GoogleFonts.poppins(
            color: Colors.white,
            fontWeight: FontWeight.w600,
