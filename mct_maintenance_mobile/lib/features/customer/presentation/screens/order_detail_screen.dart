...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  Future<void> _handlePayment() async {
    ...
    try {
      ...
      final paymentData = await _paymentRepository.initializeOrderPayment(
        orderId: orderId,
        amount: orderAmount,
      ...
      if (mounted) {
        ...
        // Naviguer immédiatement vers l'écran de statut (qui fait le polling)
        if (!mounted) return;
  ...
  Future<void> _downloadInvoice() async {
    ...
    try {
      // Télécharger le PDF
      final orderId = int.parse(widget.order['id'].toString());
      final pdfBytes = await _shopRepository.downloadInvoicePDF(orderId);
