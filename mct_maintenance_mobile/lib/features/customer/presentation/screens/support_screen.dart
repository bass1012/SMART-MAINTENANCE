...
class _SupportScreenState extends State<SupportScreen>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      if (kDebugMode)
        debugPrint('🔄 App resumed, vérification de la connexion chat...');
    ...
    try {
      ...
      _chatService.messageStream.listen((data) {
        if (mounted) {
          if (kDebugMode)
            debugPrint(
                '📨 [SupportScreen] Nouveau message reçu: ${data['message']}');
          ...
          if (!exists) {
            ...
            _chatService.addMessageToCache(data).catchError((e) {
              if (kDebugMode) debugPrint('⚠️ Erreur sauvegarde cache: $e');
            });
            _scrollToBottom();
            ...
            if (kDebugMode)
              debugPrint('⚠️ [SupportScreen] Message déjà existant, ignoré');
          }
        }
      });
          ...
          setState(() {
            for (var id in messageIds) {
              ...
              if (index != -1) {
                ...
                if (kDebugMode)
                  debugPrint(
                      '❌ [SupportScreen] Message $id NOT FOUND dans la liste');
              }
            }
          });
        }
      ...
      if (kDebugMode) debugPrint('Erreur initialisation chat: $e');
    } finally {
      if (mounted) {
        ...
        if (mounted) {
          setState(() {
            ...
            _chatService.setCachedMessages(jsonMessages).catchError((e) {
              if (kDebugMode)
                debugPrint('⚠️ Erreur sauvegarde cache fusion: $e');
            });
          });
          _scrollToBottom();
  ...
  Future<void> _refreshMessages() async {
    ...
    if (kDebugMode)
      debugPrint('🔄 [SupportScreen] Actualisation des messages...');
    ...
    try {
      ...
      if (kDebugMode) debugPrint('❌ [SupportScreen] Erreur actualisation: $e');
      if (mounted) {
  ...
  void _markMessageAsRead(int messageId) {
    if (kDebugMode)
      debugPrint('📖 [SupportScreen] Marquage message $messageId comme lu');
