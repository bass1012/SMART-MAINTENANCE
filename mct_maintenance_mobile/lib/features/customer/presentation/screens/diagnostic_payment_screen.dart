...
class _DiagnosticPaymentScreenState extends State<DiagnosticPaymentScreen> {
  ...
  Future<void> _processPayment() async {
    ...
    try {
      ...
      final paymentData = await _paymentRepository.initializeDiagnosticPayment(
      ...
      if (mounted) {
        ...
        // Afficher immédiatement le dialogue de vérification qui fait le polling
  ...
  Future<void> _pollPaymentStatus(
    ...
    while (_isPolling && _pollCount < _maxPolls && mounted) {
      ...
      try {
        final response = await _paymentRepository.verifyDiagnosticPayment(
          widget.interventionId,
        );
