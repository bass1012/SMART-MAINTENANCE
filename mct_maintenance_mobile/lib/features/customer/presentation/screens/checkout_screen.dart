    ...
    try {
      ...
      if (responseData['success']) {
        ...
        SnackBarHelper.showError(
            context, responseData['message'] ?? 'Code promo invalide');
      }
    } catch (e) {
        ...
        if (isCash) {
          ...
          if (paymentUrl != null && paymentUrl.toString().isNotEmpty) {
            try {
              ...
              // Ouvrir le paiement dans le navigateur système externe
              final Uri uri = Uri.parse(paymentUrl.toString());
              await launchUrl(uri, mode: LaunchMode.externalApplication);

              // Naviguer vers l'écran de statut (qui fait le polling)
              if (!mounted) return;
              ...
              // Si on revient ici, c'est que le paiement a échoué ou a été annulé
              if (mounted) {
                try {
