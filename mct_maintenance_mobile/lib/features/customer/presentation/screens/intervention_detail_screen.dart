...
class _InterventionDetailScreenState extends State<InterventionDetailScreen> {
  ...
  Future<void> _refreshIntervention() async {
    ...
    try {
      if (kDebugMode)
        debugPrint(
            '📡 Appel API getInterventionById(${_intervention['id']})...');
      final interventionRepository = context.read<InterventionRepository>();
      final response =
          await interventionRepository.getInterventionById(_intervention['id']);

      final newStatus = response['data']['status'];
      final oldStatus = _intervention['status'];

      if (kDebugMode) debugPrint('✅ Réponse API reçue:');
      if (kDebugMode) debugPrint('   - Ancien statut: $oldStatus');
      if (kDebugMode) debugPrint('   - Nouveau statut: $newStatus');
      ...
      if (mounted) {
        ...
        if (oldStatus != newStatus &&
          ...
          if (kDebugMode)
            debugPrint(
                '⏹️ Arrêt du timer auto-refresh (intervention terminée)');
          _refreshTimer?.cancel();
          _refreshTimer = null;
        }
      }
    } catch (e) {
      if (kDebugMode) debugPrint('❌ Erreur lors du rafraîchissement: $e');
      if (mounted) {
              ...
              child: RefreshIndicator(
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      if (status == 'client_unreachable') _buildUnreachableBanner(),
                      if (status == 'client_unreachable') const SizedBox(height: 16),
                      // Titre et priorité
                      Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
  ...
  bool _needsPayment() {
    ...
    // Pas de paiement possible pour les interventions annulées ou terminées
    if (status == 'cancelled' ||
        status == 'canceled' ||
        status == 'completed') {
      return false;
  ...
  Future<void> _confirmIntervention() async {
    ...
    if (confirmed == true) {
      ...
      try {
        ...
        if (kDebugMode) debugPrint('🔍 Réponse confirmation: $response');
        if (kDebugMode) debugPrint('🔍 response[data]: ${response['data']}');
        if (kDebugMode)
          debugPrint(
              '🔍 payment_required: ${response['data']?['payment_required']}');
        if (kDebugMode)
        ...
        if (response['data'] != null &&
          ...
          if (kDebugMode)
            debugPrint(
                '💰 Second paiement CONTRAT requis: $contractPaymentInfo');
          if (mounted) {
        ...
        if (response['data'] != null &&
          ...
          if (kDebugMode) debugPrint('💰 Second paiement requis: $paymentInfo');
  ...
  Future<void> _rescheduleIntervention(DateTime newDate) async {
    setState(() => _isLoading = true);
    try {
      final interventionId = _intervention['id'];
      final interventionRepository = context.read<InterventionRepository>();
      final response = await interventionRepository.rescheduleIntervention(
        interventionId,
        newDate,
      );
      if (mounted) {
        if (response['success'] == true) {
          SnackBarHelper.showSuccess(context, 'Intervention reprogrammée avec succès');
          _refreshIntervention();
        } else {
          SnackBarHelper.showError(context, response['message'] ?? 'Erreur lors de la reprogrammation');
          setState(() => _isLoading = false);
        }
      }
    } catch (e) {
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Technicien sur place - Vous êtes injoignable',
              ...
              if (newDate != null) {
                if (mounted) {
                  ...
                  if (time != null) {
                    ...
                    _rescheduleIntervention(finalDate);
                  }
                }
              }
            },
  ...
  Future<void> _cancelIntervention() async {
    ...
    try {
      final interventionRepository = context.read<InterventionRepository>();
      await interventionRepository.cancelIntervention(_intervention['id']);
    ...
    return Card(
      ...
      child: Container(
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(12),
          border:
              Border.all(color: const Color(0xFF0a543d).withValues(alpha: 0.3)),
        ),
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Column(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [
                      ...
                      const Color(0xFF0f7d59).withValues(alpha: 0.05),
                    ],
                  ),
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(
                    color: const Color(0xFF0a543d).withValues(alpha: 0.2),
                  ),
                ),
                child: Column(
                  children: [
