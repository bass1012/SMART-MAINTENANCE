...
import 'package:mct_maintenance_mobile/features/customer/presentation/screens/maintenance_reports_screen.dart';
...
import 'package:mct_maintenance_mobile/features/customer/presentation/screens/intervention_detail_screen.dart';
import 'package:mct_maintenance_mobile/features/customer/presentation/screens/equipments_screen.dart';
import 'package:mct_maintenance_mobile/features/customer/presentation/screens/shop_screen.dart';
...
import 'package:mct_maintenance_mobile/features/customer/presentation/screens/faq_screen.dart';
...
import 'package:mct_maintenance_mobile/features/customer/presentation/screens/warranty_screen.dart';
...
class _CustomerMainScreenState extends State<CustomerMainScreen> {
  ...
  void initState() {
    ...
    _notificationSubscription = FCMService().onNotificationTap.listen((data) {
      if (kDebugMode)
        debugPrint('🔔 [CustomerMainScreen] Notification tap reçue via stream');
      if (mounted) {
  ...
  Future<void> _checkPendingConfirmationReports() async {
    ...
    try {
      ...
      final pendingReports =
          await interventionRepository.getPendingConfirmationReports();

      if (kDebugMode)
        debugPrint(
            '✅ ${pendingReports.length} rapport(s) en attente de confirmation');

      if (pendingReports.isNotEmpty && mounted) {
        _showPendingConfirmationDialog(pendingReports.first);
      }
    } catch (e) {
      if (kDebugMode) debugPrint('❌ Erreur vérification rapports: $e');
    }
  }
  ...
  Future<void> _checkPendingDiagnosticPayments() async {
    ...
    try {
      ...
      final pendingPayments =
          await interventionRepository.getPendingDiagnosticPayments();

      if (kDebugMode)
        debugPrint('✅ ${pendingPayments.length} paiement(s) en attente');

      if (pendingPayments.isNotEmpty && mounted) {
        _showPendingPaymentNotification(pendingPayments);
      }
    } catch (e) {
      if (kDebugMode) debugPrint('❌ Erreur vérification paiements: $e');
    }
  }
  ...
  Future<void> _checkUnratedInterventions() async {
    ...
    if (!mounted) {
      if (kDebugMode)
        debugPrint('⚠️ Widget non monté, annulation vérification');
      return;
    }
    ...
    try {
      ...
      if (kDebugMode) debugPrint('📋 Détails: $unratedInterventions');
      ...
      if (filteredInterventions.isNotEmpty && mounted) {
        ...
        if (kDebugMode)
          debugPrint('ℹ️ Aucune intervention à noter ou widget non monté');
      }
    } catch (e) {
      if (kDebugMode)
        debugPrint(
  ...
  Future<void> _checkPendingNotifications() async {
    ...
    if (notificationData != null) {
      if (kDebugMode)
        debugPrint('📬 Notification en attente détectée, navigation...');
      final navigationService = NotificationNavigationService();
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          if (kDebugMode) debugPrint('👤 User chargé: role=${_user?.role}');
        });
      }

      // Ensuite charger les stats (peut échouer pour les admins)
      try {
        final statsResponse = await interventionRepository.getDashboardStats();
        if (mounted) {
          setState(() {
        ...
        if (kDebugMode)
          debugPrint('⚠️  Erreur stats (normal pour admin): $statsError');
      }
  ...
  Future<void> _loadUnreadNotifications() async {
    try {
      ...
      if (kDebugMode) debugPrint('Erreur chargement notifications: $e');
    }
  }
    ...
    return Scaffold(
      ...
      body: _isLoading
          ...
          : ResponsiveBackground(
              ...
              showGradient: true,
              gradientColors: [
                const Color(0xFF0a543d).withValues(alpha: 0.30),
                const Color(0xFF0d6b4d).withValues(alpha: 0.30),
                const Color(0xFF0f7d59).withValues(alpha: 0.30),
  ...
  Widget _buildProfileButton() {
    final hasAvatar = _user?.profileImage != null &&
        _user!.profileImage!.isNotEmpty &&
        !_avatarError;
    final imageProvider =
        hasAvatar ? AvatarHelper.buildImageProvider(_user!.profileImage) : null;
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      shape: BoxShape.circle,
                      gradient: AvatarHelper.hasAvatar(_user?.profileImage) &&
                              !_avatarError
                          ? null
                          : const LinearGradient(
                              colors: [Color(0xFF0a543d), Color(0xFF0d6b4d)],
                      ...
                      image: AvatarHelper.hasAvatar(_user?.profileImage) &&
                              !_avatarError
                          ? DecorationImage(
                              image: AvatarHelper.buildImageProvider(
                                  _user!.profileImage)!,
                              fit: BoxFit.cover,
                              onError: (exception, stackTrace) {
                                if (mounted)
                    ...
                    child: AvatarHelper.hasAvatar(_user?.profileImage) &&
                            !_avatarError
                        ? null
                        : Center(
                            child: Text(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [
                  Colors.white,
                  const Color(0xFF0a543d).withValues(alpha: 0.02),
                ],
              ),
              borderRadius: BorderRadius.circular(24),
            ),
