...
class _InterventionsListScreenState extends State<InterventionsListScreen> {
  ...
  List<Map<String, dynamic>> _cachedFilteredInterventions = [];
  ...
  Future<void> _loadPendingDiagnosticPayments() async {
    try {
      ...
      if (mounted) {
        setState(() {
          // Exclure les interventions annulées de la liste des paiements en attente
          _pendingDiagnosticPayments = pending.where((i) {
      ...
      if (kDebugMode)
        debugPrint('❌ Erreur chargement paiements diagnostic en attente: $e');
    }
  }
  ...
  Future<void> _loadPendingSubscriptionPayments() async {
    try {
      ...
      final pending =
          await subscriptionRepository.getPendingSubscriptionPayments();
      if (mounted) {
        setState(() {
      ...
      if (kDebugMode)
        debugPrint('❌ Erreur chargement paiements souscription en attente: $e');
    }
  }
  ...
  Future<void> _loadInterventions() async {
    ...
    try {
      ...
      if (kDebugMode)
        debugPrint(
            '🔍 [Interventions] UserData keys: ${userData?.keys.toList()}');
      if (kDebugMode)
        debugPrint('🔍 [Interventions] CustomerId extrait: $customerId');
      ...
      final response =
          await interventionRepository.getInterventions(customerId: customerId);

      if (mounted) {
        setState(() {
          final rawData = response['data'];
          final List<dynamic> rawInterventions = rawData is List
              ? rawData
              : (rawData is Map ? (rawData['interventions'] as List? ?? []) : []);
          _interventions = List<Map<String, dynamic>>.from(rawInterventions);
          _applyFilter();
          _isLoading = false;
        });
      }
  ...
  void _applyFilter() {
    if (_filterStatus == 'all') {
      _cachedFilteredInterventions = _interventions;
    } else {
      _cachedFilteredInterventions =
          _interventions.where((i) => i['status'] == _filterStatus).toList();
    }
  }
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return TweenAnimationBuilder<double>(
                                  key: ValueKey(
                                      '${intervention['id']}_$_filterStatus'),
                                  tween: Tween(begin: 0.0, end: 1.0),
                                  duration: Duration(
                                      milliseconds:
                                          180 + (index * 35).clamp(0, 280)),
                                  ...
                                  child: _buildInterventionCard(intervention),
                                );
                              },
                            ),
                          ),
  ...
  Future<void> _cancelIntervention(Map<String, dynamic> intervention) async {
    try {
      ...
      final interventionRepository = context.read<InterventionRepository>();
      await interventionRepository.cancelIntervention(id);
  ...
  Widget _buildFilterChip(String label, String value) {
    ...
    return FilterChip(
      ...
      selectedColor: const Color(0xFF0a543d).withValues(alpha: 0.2),
        ...
        child: Column(
          ...
          children: [
            Icon(
              ...
              size: 80,
              color: const Color(0xFF0a543d).withValues(alpha: 0.6),
            ),
            const SizedBox(height: 16),
            Text(
              'Aucune intervention',
