...
class _QuoteDetailScreenState extends State<QuoteDetailScreen> {
  ...
  Future<void> _downloadQuotePDF(String quoteId) async {
    ...
    try {
      final pdfBytes = await _contractRepository.downloadQuotePDF(quoteId);
      ...
      final fileName = 'devis-${_quote.reference}.pdf';
  ...
  Future<void> _processAcceptQuote({
    ...
    try {
      ...
      final acceptResponse = await _contractRepository.acceptQuote(
        _quote.id,
      ...
      if (mounted) {
        ...
        if (directOrderId != null) {
          ...
          try {
            ...
            final ordersResponse = await apiService.get('/api/orders');
            final data = json.decode(ordersResponse.body);
  ...
  Future<void> _rejectQuote() async {
    ...
    try {
      ...
      final updatedQuote = await _contractRepository.rejectQuote(_quote.id, reason);
  ...
  Future<void> _payNow() async {
    ...
    try {
      ...
      if (order.isNotEmpty && order['id'] != null) {
        ...
        if (_quote.paymentType == 'split' || _quote.firstPaymentAmount != null || _quote.secondPaymentAmount != null) {
          final firstPaymentAmount = _quote.firstPaymentAmount ?? (_quote.amount / 2).ceilToDouble();
          final secondPaymentAmount = _quote.secondPaymentAmount ?? (_quote.amount - firstPaymentAmount);

          if (_quote.firstPaymentStatus == 'paid' || _quote.secondPaymentStatus == 'paid') {
            // Si le premier paiement est déjà fait, payer le solde restant
            paymentAmount = secondPaymentAmount > 0 ? secondPaymentAmount : firstPaymentAmount;
            paymentStep = 2;
          } else {
  ...
  Future<void> _refreshQuote() async {
    try {
      final updatedQuote = await _contractRepository.getQuoteDetails(_quote.id);
      if (mounted) {
        setState(() {
          _quote = updatedQuote;
        });
      ...
      if (kDebugMode) debugPrint('⚠️ Erreur rafraîchissement devis: $e');
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _downloadQuotePDF(_quote.id),
          ),
        ],
      ),
      body: Container(
                          ...
                          children: [
                            ...
                            if (_quote.scheduledDate != null)
                              _buildInfoRow(
                                ...
                                '${dateFormat.format(_quote.scheduledDate!)} à ${DateFormat("HH'h'mm").format(_quote.scheduledDate!)}',
                              ),
                            if (_quote.scheduledDate != null &&
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  TextButton.icon(
                                    onPressed: () => _downloadQuotePDF(_quote.id),
                                    icon: const Icon(Icons.picture_as_pdf, size: 18, color: Color(0xFF0a543d)),
                                    label: const Text(
                                      'Télécharger PDF',
                                      style: TextStyle(
                                        color: Color(0xFF0a543d),
                              ...
                              Padding(
                                ...
                                child: Column(
                                  children: [
                                    _buildSummaryRow('TOTAL H.T.V.A.', _quote.subtotal),
                                    const SizedBox(height: 8),
                                    _buildSummaryRow('TVA 18%', _quote.taxAmount),
                                    const SizedBox(height: 8),
                    ...
                    if (_quote.status == 'accepted')
                      Column(
                        children: [
                          ...
                          // Bouton "Payer maintenant" pour tout paiement non complété
