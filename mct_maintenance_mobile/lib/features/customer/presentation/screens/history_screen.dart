...
class _HistoryScreenState extends State<HistoryScreen>
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      // Charger les vraies données depuis l'API via les repositories
      final interventionsResponse =
          await _interventionRepository.getInterventions();
      final ordersResponse = await _shopRepository.getOrders();
      final quotesResponse = await _contractRepository.getCustomerQuotesRaw();

      if (mounted) {
        setState(() {
          // Stocker les données brutes avec vérification de type
          final interventionsRaw = interventionsResponse['data'];
          final interventionsList = interventionsRaw is List
              ? interventionsRaw
              : (interventionsRaw is Map
                  ? (interventionsRaw['interventions'] as List? ?? [])
                  : []);
          _interventionsRawData =
              interventionsList.whereType<Map<String, dynamic>>().toList();
  ...
  List<HistoryItem> _parseInterventions(Map<String, dynamic> response) {
    try {
      ...
      // L'API retourne data comme Map { interventions: [] } ou directement comme List
      final List<dynamic> interventionsData = rawData is List
          ? rawData
          : (rawData is Map ? (rawData['interventions'] as List? ?? []) : []);
      if (interventionsData.isEmpty) return [];
