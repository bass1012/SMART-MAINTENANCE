...
class _EquipmentsScreenState extends State<EquipmentsScreen> {
  ...
  void initState() {
    super.initState();
    _equipmentRepository = context.read<EquipmentRepository>();
    _loadEquipments();
  }
  ...
  Future<void> _loadEquipments() async {
    try {
      ...
      final equipments = await _equipmentRepository.getMyEquipments();
  ...
  Future<void> _addEquipment(Map<String, dynamic> data) async {
    try {
      final response = await _equipmentRepository.addEquipment(data);
  ...
  Future<void> _updateEquipment(int id, Map<String, dynamic> data) async {
    try {
      final response = await _equipmentRepository.updateEquipment(id, data);
      ...
      if (response['success'] == true) {
        ...
        if (mounted) {
          SnackBarHelper.showError(
              context, response['message'] ?? 'Erreur lors de la modification');
        }
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _deleteEquipment(int id) async {
    try {
      final response = await _equipmentRepository.deleteEquipment(id);
      ...
      if (response['success'] == true) {
        ...
        if (mounted) {
          SnackBarHelper.showError(
              context, response['message'] ?? 'Erreur lors de la suppression');
        }
      }
    } catch (e) {
      if (mounted) {
