...
class _InvoicesScreenState extends State<InvoicesScreen> {
  ...
  List<Invoice> _cachedFilteredInvoices = [];
  int _paidCount = 0;
  int _overdueCount = 0;
  ...
  void _updateCache() {
    _paidCount = _invoices.where((i) => i.status == 'paid').length;
    _overdueCount = _invoices.where((i) => i.status == 'overdue').length;
    _applyFilter();
  }

  void _applyFilter() {
    _cachedFilteredInvoices = _filterStatus == 'all'
        ? _invoices
  ...
  Future<void> _loadInvoices() async {
    ...
    try {
      ...
      final response = await _paymentRepository.getPaymentHistory();
  ...
  List<Invoice> get _filteredInvoices => _cachedFilteredInvoices;
