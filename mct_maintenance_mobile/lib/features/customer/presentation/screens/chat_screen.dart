...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  Future<void> _initChat() async {
    if (kDebugMode) debugPrint('🎬 [ChatScreen] Initialisation du chat...');
    if (kDebugMode)
      debugPrint(
    ...
    if (userDataString != null) {
      try {
        ...
        if (kDebugMode)
          debugPrint(
              '🔍 [ChatScreen] UserData keys: ${userData.keys.toList()}');
        if (kDebugMode) debugPrint('🔍 [ChatScreen] UserId extrait: $_userId');
      } catch (e) {
        if (kDebugMode)
    ...
    if (kDebugMode) debugPrint('🔌 [ChatScreen] Appel de ensureConnected()...');
    ...
    setState(() {
      ...
      if (kDebugMode)
        debugPrint('🎨 [ChatScreen] setState: _isConnected = $_isConnected');
    });
    ...
    _messageSubscription = _chatService.messageStream.listen((data) {
      ...
      if (!exists) {
        ...
        if (kDebugMode)
          debugPrint('⚠️ [ChatScreen] Message déjà existant, ignoré');
      }
    });
    ...
    _chatService.messagesReadStream.listen((messageIds) {
      if (kDebugMode)
        debugPrint(
            '📖 [ChatScreen] ${messageIds.length} messages marqués comme lus');
      setState(() {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          if (kDebugMode)
            debugPrint(
                '📜 [ChatScreen] Total après fusion: ${allMessages.length}');
      ...
      if (kDebugMode) debugPrint('❌ Erreur chargement historique: $e');
      setState(() {
        _isLoading = false;
      });
    }
  ...
  Widget build(BuildContext context) {
    if (kDebugMode)
      debugPrint('🎨 [ChatScreen.build] Le widget est reconstruit');
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted && _chatService.isConnected != _isConnected) {
          setState(() {
            _isConnected = _chatService.isConnected;
            if (kDebugMode)
              debugPrint('✅ [PostFrameCallback] État forcé à: $_isConnected');
          });
        }
      });
    }
  ...
  Widget _buildMessageBubble(ChatMessage message) {
    ...
    final time = DateFormat("HH'h'mm").format(message.createdAt);
