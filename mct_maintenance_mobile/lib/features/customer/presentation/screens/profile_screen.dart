...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _deleteAccount() async {
    ...
    if (confirmed == true && mounted) {
      try {
        ...
        if (mounted) {
          SnackBarHelper.showError(context, 'Erreur lors de la suppression du compte: $e');
        }
      }
    }
  }
  ...
  Future<void> _pickImage() async {
    ...
    if (_isPickingImage) {
      if (kDebugMode) debugPrint('⚠️ Sélection d\'image déjà en cours, ignorer le double tap');
      return;
    }
    ...
    try {
      ...
      if (source != null && mounted) {
        try {
          ...
          if (image != null && mounted) {
            if (kDebugMode) debugPrint('✅ Image sélectionnée: ${image.path}');
            setState(() {
              _selectedImage = File(image.path);
            });
            if (kDebugMode) debugPrint('✅ _selectedImage défini: ${_selectedImage?.path}');
            ...
            if (kDebugMode) debugPrint('⚠️ Aucune image sélectionnée ou widget non monté');
          }
        } catch (e) {
          if (kDebugMode) debugPrint('❌ Erreur lors de la sélection de l\'image: $e');
          if (mounted) {
      ...
      if (kDebugMode) debugPrint('❌ Erreur générale dans _pickImage: $e');
      if (mounted) {
        SnackBarHelper.showError(context, e.toString());
      }
    } finally {
  ...
  Future<void> _saveProfile() async {
    if (kDebugMode) debugPrint('🚀 _saveProfile appelé');
    if (kDebugMode) debugPrint('📝 _selectedImage: ${_selectedImage?.path}');

    if (!_formKey.currentState!.validate()) {
      if (kDebugMode) debugPrint('❌ Validation du formulaire échouée');
      return;
    }
    ...
    try {
      ...
      if (kDebugMode) debugPrint('📦 updateData final: $updateData');

      final authRepository = context.read<AuthRepository>();

      // Upload de l'image si une nouvelle image a été sélectionnée
      if (_selectedImage != null) {
        if (kDebugMode) debugPrint('🖼️ Une nouvelle image a été sélectionnée, upload en cours...');
        try {
          if (kDebugMode) debugPrint('📤 Upload de l\'avatar en cours...');
          if (kDebugMode) debugPrint('📁 Chemin du fichier: ${_selectedImage!.path}');

          final imageUrl = await authRepository.uploadAvatar(_selectedImage!.path);
          if (kDebugMode) debugPrint('✅ Avatar uploadé avec succès !');
          if (kDebugMode) debugPrint('🔗 URL retournée: $imageUrl');

          // Extraire uniquement le nom du fichier depuis l'URL s'il ne s'agit pas de base64
          String profileImageValue = imageUrl;
          if (!AvatarHelper.isBase64Url(imageUrl) && imageUrl.contains('/')) {
            profileImageValue = imageUrl.split('/').last;
          }

          if (kDebugMode) debugPrint('📝 Valeur image profil finale: $profileImageValue');
          if (kDebugMode) debugPrint('🖼️ Ancienne image: ${_user?.profileImage}');

          updateData['profile_image'] = profileImageValue;
          if (kDebugMode) debugPrint('📦 Données à envoyer: $updateData');
        } catch (e) {
          if (kDebugMode) debugPrint('❌ Erreur upload avatar: $e');
          if (mounted) {
            SnackBarHelper.showWarning(
              context,
      ...
      if (kDebugMode) debugPrint('🚀 Mise à jour du profil...');
      if (kDebugMode) debugPrint('📦 updateData final: $updateData');

      // Appel API pour mettre à jour le profil
      final response = await authRepository.updateProfile(updateData);
      if (kDebugMode) debugPrint('✅ Réponse updateProfile: $response');
              ...
              gradient: LinearGradient(
                colors: enabled
                    ? [
                        ...
                        const Color(0xFF0d6b4d).withValues(alpha: 0.1)
                      ]
                    : [
            ...
            onPressed: () async {
              ...
              try {
                ...
                try {
                  ...
                  if (kDebugMode) debugPrint('ℹ️ Erreur logout ignorée (compte déjà supprimé): $e');
                }
