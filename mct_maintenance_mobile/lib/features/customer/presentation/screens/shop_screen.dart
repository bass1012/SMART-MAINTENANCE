...
class _ShopScreenState extends State<ShopScreen> {
  ...
  Future<void> _loadPremiumOffer() async {
    try {
      ...
      final offers = await serviceRepository.getMaintenanceOffers();
      ...
      if (kDebugMode) debugPrint('Erreur lors du chargement de l\'offre premium: $e');
    }
  }
  ...
  Future<void> _loadBrands() async {
    ...
    try {
      ...
      final response = await shopRepository.getBrands();
      ...
      if (mounted) {
        ...
        if (kDebugMode) debugPrint('Erreur lors du chargement des marques: $e');
      }
    }
  }
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        final bool isShopMaintenance = response['shop_maintenance'] == true ||
            (productsData.isNotEmpty &&
                productsData.every((json) => json['actif'] == false));
        setState(() {
          _products =
              productsData.map((json) => ProductModel.fromJson(json)).toList();
          _isShopMaintenance = isShopMaintenance;
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  bool get _isInMaintenanceMode =>
      _isShopMaintenance ||
      (_products.isNotEmpty && _products.every((p) => p.actif == false));
    ...
    return SupportFabWrapper(
      child: Scaffold(
        ...
        body: SimpleResponsiveBackground(
          ...
          child: Column(
            children: [
              // Header avec filtres par marque (masqué en mode maintenance)
                ...
                child: _isLoading
                    ...
                    : (_isShopMaintenance ||
                            (_products.isNotEmpty &&
                                _products.every((p) => p.actif == false)))
                        ? _buildShopMaintenanceView()
  ...
  Widget _buildShopMaintenanceView() {
    return Center(
      child: SingleChildScrollView(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: const Color(0xFF0a543d).withValues(alpha: 0.15),
              width: 1.5,
            ),
          ),
          child: Column(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: const LinearGradient(
                    colors: [Color(0xFF0a543d), Color(0xFF137e5e)],
              ...
              Text(
                'Notre boutique en ligne de climatisation fait actuellement peau neuve pour vous offrir une expérience optimale et de nouveaux catalogues.',
                textAlign: TextAlign.center,
                style: GoogleFonts.poppins(
                  fontSize: 13.5,
                  height: 1.55,
              ...
              const SizedBox(height: 12),
              Text(
                'Pendant cette période, nos services d\'entretien, de diagnostic et de réparation restent 100% opérationnels.',
                textAlign: TextAlign.center,
                style: GoogleFonts.poppins(
                  fontSize: 13,
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: product.inStock
                            ? [
                                BoxShadow(
                                  color:
                                      const Color(0xFF0a543d).withValues(alpha: 0.4),
                                  blurRadius: 12,
                                  offset: const Offset(0, 6),
                                ),
                              ]
