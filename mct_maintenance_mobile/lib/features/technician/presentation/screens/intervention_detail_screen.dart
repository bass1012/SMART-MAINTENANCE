...
class _InterventionDetailScreenState extends State<InterventionDetailScreen> {
  ...
  final bool _splitRequired = true; // Le scan est obligatoire par défaut
  ...
  void initState() {
    ...
    if (kDebugMode)
      debugPrint('📍 initState - Intervention ${_intervention['id']} chargée');
    if (kDebugMode)
      debugPrint('   🔍 DEBUG - customer: ${_intervention['customer']}');
    if (kDebugMode) debugPrint('   🔍 DEBUG - date: ${_intervention['date']}');
    if (kDebugMode) debugPrint('   🔍 DEBUG - time: ${_intervention['time']}');
    if (kDebugMode) debugPrint('   🔍 DEBUG - type: ${_intervention['type']}');
    print(
        '   🔍 DEBUG - climatiseur_type: ${_intervention['climatiseur_type']}');
    if (kDebugMode)
      debugPrint(
          '   🔍 DEBUG - Keys disponibles: ${_intervention.keys.toList()}');
  ...
  Future<void> _performAction(String action) async {
    ...
    final int interventionId = _intervention['id'] is int
        ? _intervention['id']
        : int.parse(_intervention['id'].toString());
    ...
    try {
      ...
      switch (action) {
        case 'accept':
          response =
              await interventionRepository.acceptIntervention(interventionId);
          break;
        case 'on_the_way':
          response = await interventionRepository
        ...
        case 'in_progress':
          response =
              await interventionRepository.startIntervention(interventionId);
          break;
        case 'complete':
          response =
              await interventionRepository.completeIntervention(interventionId);
          break;
        default:
  ...
  Future<void> _goToReport() async {
    ...
    if (requiresDiagnosticReport) {
      ...
      // Pour toute intervention en cours, ouvrir le rapport d'intervention (isInitialStep = false)
    ...
    if (result == true && mounted) {
      ...
      if (kDebugMode) {
        debugPrint(
            '🔄 Rapport soumis, rechargement complet de l\'intervention...');
      }
      setState(() => _isLoading = true);
      ...
      try {
        ...
        if (response['success'] && response['data'] != null) {
          ...
          if (kDebugMode) debugPrint('✅ Intervention rechargée:');
          if (kDebugMode) {
            debugPrint('   - status: ${updatedIntervention['status']}');
          }
          print(
        ...
        if (kDebugMode) debugPrint('❌ Erreur rechargement: $e');
        setState(() => _isLoading = false);
      ...
      MaterialPageRoute(
        builder: (context) => SplitScanScreen(
          ...
          onSplitScanned: (scanResult) {
            if (kDebugMode)
              debugPrint('✅ Split scanné: ${scanResult.split.splitCode}');
          },
        ),
      ),
    );
  ...
  Future<void> _loadInterventionFromCache() async {
    ...
    try {
      ...
      final interventionRepository = context.read<InterventionRepository>();
      final response =
          await interventionRepository.getTechnicianInterventions();
      ...
      if (mounted && response['data'] != null) {
        ...
        if (kDebugMode) debugPrint('✅ Intervention mise à jour depuis cache:');
        if (kDebugMode)
          debugPrint('   - status: ${updatedIntervention['status']}');
        print(
            '   - report_submitted_at: ${updatedIntervention['report_submitted_at']}');
      ...
      if (kDebugMode)
        debugPrint('❌ Erreur lors du rechargement depuis cache: $e');
    }
  }
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [
                const Color(0xFF0a543d),
                const Color(0xFF0a543d).withValues(alpha: 0.8),
              ],
            ),
          ),
          child: Stack(
        ...
        child: Column(
          ...
          children: [
              ...
              Row(
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () async {
                      String phoneStr = _intervention['customer_phone'].toString();
                      if (phoneStr.startsWith('225')) {
                        phoneStr = '+$phoneStr';
                      }
                      final Uri phoneUri = Uri(
                          scheme: 'tel', path: phoneStr);
            ...
            _buildInfoRow(
              ...
              'Date',
              '${_intervention['date']} à ${_intervention['time']?.toString().replaceAll(':', 'h') ?? ''}',
            ),
            const SizedBox(height: 12),
            _buildInfoRow(
              Icons.category,
  ...
  Future<void> _openMap() async {
    ...
    try {
      ...
      if (kDebugMode) debugPrint('🗺️ Ouverture de la carte avec URL: $mapUrl');

      // Essayer de lancer l'URL
      final canLaunch = await canLaunchUrl(mapUrl);
      if (kDebugMode) debugPrint('🗺️ canLaunchUrl result: $canLaunch');
      ...
      if (canLaunch) {
        ...
        if (kDebugMode) debugPrint('🗺️ launchUrl result: $launched');
        ...
        if (Platform.isAndroid) {
          ...
          if (kDebugMode) debugPrint('🗺️ Tentative avec geo: $geoUrl');
      ...
      if (kDebugMode)
        debugPrint('❌ Erreur lors de l\'ouverture de la carte: $e');
      if (mounted) {
        SnackBarHelper.showError(
          context,
