          ...
          _allInterventions = (response['data'] as List? ?? [])
            ...
            if (item['scheduled_date'] != null) {
              try {
                ...
                formattedTime = DateFormat("HH'h'mm").format(dateTime);
              } catch (e) {
                formattedDate = item['scheduled_date'].toString().split(' ')[0];
                formattedTime = (item['scheduled_time'] ?? '00:00').toString().replaceAll(':', 'h');
              }
            } else {
              formattedDate =
                  item['date'] ?? DateTime.now().toString().split(' ')[0];
              formattedTime = (item['scheduled_time'] ?? item['time'] ?? '00:00').toString().replaceAll(':', 'h');
            }
  ...
  void _filterInterventions() {
    ...
    setState(() {
      _filteredInterventions = _allInterventions
          .where((intervention) => statuses.contains(intervention['status']))
          .toList();
    });
  }
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return TweenAnimationBuilder<double>(
                              key: ValueKey(
                                  '${intervention['id']}_${_tabController.index}'),
