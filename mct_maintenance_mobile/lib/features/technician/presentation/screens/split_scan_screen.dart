...
import 'package:mct_maintenance_mobile/services/split_service.dart';
...
class _SplitScanScreenState extends State<SplitScanScreen> {
  ...
  Future<void> _processQRCode(String qrData) async {
    ...
    if (kDebugMode) debugPrint('🔍 Code split scanné: $splitCode');
...
class _ManualSplitSelectionSheetState
  ...
  Future<void> _loadSplits() async {
    ...
    if (widget.customerId != null) {
      ...
      if (kDebugMode) debugPrint('📋 Splits par customerId: ${splits.length}');
    }
    ...
    if (splits.isEmpty) {
      ...
      if (kDebugMode) debugPrint('📋 Splits par interventionId: ${splits.length}');
    }

    if (splits.isEmpty) {
      // 3. Fallback: tous les splits
      splits = await _splitService.getAllSplits();
      if (kDebugMode) debugPrint('📋 Tous les splits: ${splits.length}');
    }
