...
import 'package:mct_maintenance_mobile/features/technician/presentation/screens/edit_profile_screen.dart';
...
class _TechnicianProfileScreenState extends State<TechnicianProfileScreen> {
  ...
  Future<void> _loadStats() async {
    try {
      final response = await _interventionRepository.getTechnicianStats();
      ...
      if (kDebugMode) debugPrint('⚠️ Erreur chargement stats: $e');
    }
  }
  ...
  Future<void> _pickAndUploadImage() async {
    try {
      ...
      if (kDebugMode) debugPrint('📸 [PROFILE] Image sélectionnée: ${image.path}');
      setState(() => _isUploading = true);

      final imageFilename = await _authRepository.uploadAvatar(image.path);
      if (kDebugMode) debugPrint('📸 [PROFILE] Filename reçu du serveur: $imageFilename');
      ...
      if (kDebugMode) debugPrint('❌ [PROFILE] Erreur upload: $e');
      if (mounted) {
