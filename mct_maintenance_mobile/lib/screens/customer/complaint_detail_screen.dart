...
class _ComplaintDetailScreenState extends State<ComplaintDetailScreen> {
  ...
  Future<void> _loadComplaintDetails() async {
    try {
      final complaintDetails = await _apiService.getComplaintDetails(_complaint.id);
      if (mounted) {
        setState(() {
    ...
    return Scaffold(
      ...
      body: _isLoading
          ...
          : _error != null
                        ...
                        Row(
                          children: [
                            Container(
                              ...
                              child: Text(
                                _formatPriority(_complaint.priority),
                                style: TextStyle(
                                  color: priorityColor,
                                  fontSize: 12,
            ...
            if (_complaint.status == 'resolved' && _complaint.resolutionNotes != null) ...[
              Card(
                elevation: 2,
            ...
            // Notes de suivi
            if (_complaint.notes != null && _complaint.notes!.isNotEmpty) ...[
              Card(
                elevation: 2,
                child: Padding(
                  padding: const EdgeInsets.all(16),
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      ..._complaint.notes!.map((note) => _buildNoteItem(note, dateFormat)),
                    ],
                  ),
                ),
              ),
  ...
  Widget _buildNoteItem(dynamic note, DateFormat dateFormat) {
    ...
    final isStaff = note.authorRole == 'admin' || note.authorRole == 'technician';
