const InstallationService = require('../models/InstallationService');

// Récupérer tous les services d'installation
exports.getAllInstallationServices = async (req, res) => {
  try {
    const services = await InstallationService.findAll({
...
exports.getActiveInstallationServices = async (req, res) => {
  try {
    const services = await InstallationService.findAll({
      where: { isActive: true },
      order: [['created_at', 'DESC']]
...
exports.getInstallationServiceById = async (req, res) => {
  try {
    const service = await InstallationService.findByPk(req.params.id);
    if (!service) {
...
exports.createInstallationService = async (req, res) => {
  try {
...
exports.updateInstallationService = async (req, res) => {
  try {
    const service = await InstallationService.findByPk(req.params.id);
    if (!service) {
      return res.status(404).json({ message: 'Service non trouvé' });
...
exports.deleteInstallationService = async (req, res) => {
  try {
    const service = await InstallationService.findByPk(req.params.id);
    if (!service) {
      return res.status(404).json({ message: 'Service non trouvé' });
...
exports.toggleInstallationServiceStatus = async (req, res) => {
  try {
    const service = await InstallationService.findByPk(req.params.id);
    if (!service) {
      return res.status(404).json({ message: 'Service non trouvé' });
